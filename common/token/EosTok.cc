...
EosTok::Write(const std::string& key)
{
  ...
  std::string rkey = std::to_string(share->seed()) + key + std::to_string(share->seed());
...
{
  std::string nkey = std::to_string(share->seed()) + key + std::to_string(share->seed());
  std::string nserialized = share->serialized();
...
int 
EosTok::VerifyOrigin(const std::string& host, const std::string& name, const std::string& prot)
{
