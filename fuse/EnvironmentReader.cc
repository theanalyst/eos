  ...
  while(!shutdown) {
    if(!requestQueue.empty()) {
      ...
      if(it == pendingRequests.end()) {
        eos_static_crit("EnvironmentReader queue corruption, unable to find entry for pid %d", request.pid);
      }
      else {
...
EnvironmentResponse EnvironmentReader::stageRequest(pid_t pid) {
  ...
  eos_static_debug("Queueing request to read environment for pid %d, notifying workers", pid);
