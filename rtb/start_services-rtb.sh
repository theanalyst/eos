...
docker run -dit -h eos-mq-test.eoscluster.cern.ch -p $HOSTIP:$Pmgm:$Pmgm --name eos-mq-test --net=$NET $image
docker cp eos-mq-test:/etc/sysconfig/eos $TTTsysConfig
#sed "s/eos-mq-test.eoscluster.cern.ch:1097/$(uname -n):$Pmgm/" -i $TTTsysConfig
sed "s/eos-mgm-test.eoscluster.cern.ch/$(uname -n)/" -i $TTTsysConfig
sed "s/eos-mq-test.eoscluster.cern.ch:1097/$(uname -n):$Pmq/" -i $TTTsysConfig
sed "/EOS_INSTANCE_NAME/aexport EOS_FS_FULL_SIZE_IN_GB=1" -i $TTTsysConfig
docker cp $TTTsysConfig eos-mq-test:/etc/sysconfig/eos
docker cp eos-mq-test:/etc/xrd.cf.mq $TTT1
sed "s/xrd.port 1097/xrd.port $Pmq/" -i $TTT1
docker cp $TTT1 eos-mq-test:/etc/xrd.cf.mq
...
docker run --privileged -dit  --name eos-mgm-test --net=$NET $image
...
docker cp $TTTsysConfig eos-mgm-test:/etc/sysconfig/eos
docker cp eos-mgm-test:/usr/sbin/service $TTT2
sed "s/VERSION=/exit 0 #V ERSION=/" -i $TTT2
docker cp $TTT2 eos-mgm-test:/usr/sbin/service

docker cp /etc/krb5.keytab eos-mgm-test:/etc/eos.krb5.keytab
docker cp /etc/krb5.conf eos-mgm-test:/etc/krb5.conf


docker cp eos-mgm-test:/etc/xrd.cf.mgm $TTT1
...
[[ $with_qdb == 1 ]] && { 
    ...
    sed "s/eos-qdb-test.eoscluster.cern.ch/$(uname -n)/g" -i $TTT1
...
#	      docker volume create --driver local --opt o=size=10M  --opt type=tmpfs --opt device=tmpfs $v
    ...
    docker run $qqq --privileged -dit --name eos-qdb-test -p $HOSTIP:7777:7777  --net=$NET $image
    $cleanFS || {
        docker cp eos-mgm-test:/eos_qdb_setup.sh $TTT2
        sed "s/quarkdb-create/#quarkdb-create/" -i $TTT2
        docker cp $TTT2 eos-qdb-test:/eos_qdb_setup.sh
    }
...
if $nativeMGM; then
    docker cp eos-mgm-test:/eos_mgm_setup.sh /
    docker cp eos-mgm-test:/eos_mgm_fs_setup.sh /
    cp $TTT1 /etc/xrd.cf.mgm
    cp -p $TTTsysConfig /etc/sysconfig/eos
    cp -p /etc/krb5.keytab /etc/eos.krb5.keytab
    docker cp eos-mgm-test:/etc/eos.keytab /etc/eos.keytab
    chown daemon /etc/eos.krb5.keytab /etc/eos.keytab
    DXM=
    /eos_mgm_setup.sh
else
    docker cp $TTT1 eos-mgm-test:/etc/xrd.cf.mgm
    DXM="docker exec -t -i eos-mgm-test "
    docker exec -di eos-mgm-test /eos_mgm_setup.sh
...
do
  ...
  docker cp eos-fst${i}-test:/eos_fst_setup.sh $TTT1
  sed "s/eos-mgm-test.eoscluster.cern.ch/$(uname -n)/" -i $TTT1
...
#true && for n in user; do
    ...
    docker exec -i $CLT sed -i "s/eos-mgm-test.eoscluster.cern.ch/$(uname -n)/" -i /etc/eos/fuse.conf
    true && {
...
EOSFUSECONFIG
	...
	#docker exec -i eos-client-test mkdir -p /fos
	#docker exec -di eos-client-test mount -t fuse eosxd -ofsname=$n /fos
    docker exec -di $CLT /bin/bash -c 'mkdir /eos1/; mount -t fuse eosxd /eos1/'
    }
done

false && {
    docker cp $CLT:/etc/eos/fuse.conf $TTT1
    sed "s/eos-mgm-test.eoscluster.cern.ch/$(uname -n)/" -i $TTT1
    docker cp $TTT1 $CLT:/etc/eos/fuse.conf
    docker exec -i eos-mgm-test eos mkdir /eos/dockertest
    docker exec -i eos-mgm-test eos chmod 2777 /eos/dockertest
    docker exec -di $CLT mkdir /eos/dockertest
    true && docker exec -di $CLT mount -t fuse eosxd /eos/dockertest
}
