...
struct {
    int raclBit;
    char eosChr;
} raclEosPerms[] = { {RICHACE_READ_DATA, 'r'}, {RICHACE_WRITE_DATA, 'w'}, {RICHACE_EXECUTE, 'x'}, {RICHACE_WRITE_ACL, 'm'},
		      {RICHACE_APPEND_DATA, 'u'}, {RICHACE_DELETE, 'd'}, {RICHACE_WRITE_OWNER, 'c'} };
...
eos2racl(const char *eosacl, int mode) {
	...
	switch (uge[0]) {		/* qualifier type, just check first char */
		...
		if (qlf_num < 0) {
		    rc = getpwnam_r((const char *) qlf, &p, buf, sizeof(buf), &pp);
		 } else {
		    rc = getpwuid_r(qlf_num, &p, buf, sizeof(buf), &pp);
		}
		if (pp == NULL)
		    eos_static_debug("qlf=%s pp=%p &p=%p rc=%d pw_name '%s' pw_uid %d", qlf, pp, &p, rc, p.pw_name, p.pw_uid);
		else
		    ace->e_id = pp->pw_uid;
		break;
		...
		if (qlf_num < 0)
		    rc = getgrnam_r((const char *) qlf, &g, buf, sizeof(buf), &gg);
		else
		    rc = getgrgid_r(qlf_num, &g, buf, sizeof(buf), &gg);
		if (gg != NULL)
		    ace->e_id = gg->gr_gid;
		break;
  ...
  if (!local_getxattr) {
    {
      ...
      if (!rc) {
		...
		if (map.count("user.acl") == 0 || map["user.acl"].length() == 0) rc = ENOATTR;
		    ...
		    eos_static_debug("racl getxattr %d: %s", value.length(), escape(value).c_str());
		}
	      }
	      else
#endif /*RICHACL_FOUND*/
