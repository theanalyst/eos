...
metad::lookup(fuse_req_t req, fuse_ino_t parent, const char* name)
{
  ...
  if (pmd->data.id() == parent) {
    ...
    if (!strcmp(name, "..")) {
      pmd->Locker().UnLock();
      shared_md ppmd = get(req, pmd->data.pid(), "", false);
      pmd->Locker().Lock();
      return ppmd;
    }
	...
	if (pmd->local_enoent().count(name)) {
        ...
        if (pmd->get_todelete().count(eos::common::StringConversion::EncodeInvalidUTF8(
          ...
          if (EOS_LOGS_DEBUG) {
            eos_static_debug("in deletion list %016lx name=%s", pmd->data.id(), name);
          }
    ...
    if (md) {
      md->Locker().Lock();
      std::string fullpath = pmd->data.fullpath();
      ...
      fullpath += name;
      md->data.set_fullpath(fullpath.c_str());
      md->Locker().UnLock();
      pmd->Locker().Lock();
    } else {
      md = std::make_unique<mdx>();
...
metad::mdx::convert(struct fuse_entry_param& e, double lifetime)
{
  ...
  auto attrMap = data.attr();
  e.ino = data.id();
  e.attr.st_dev = 0;
  e.attr.st_ino = data.id();
  e.attr.st_mode = data.mode();
  e.attr.st_nlink = data.nlink();
  ...
  if (attrMap.count(k_mdino)) {
    ...
    if (!tmd->data.id()) {
      ...
      eos_static_err("converting hard-link %s target inode %#lx remote %#lx not in cache, nlink set to %d",
                     data.name().c_str(), local_ino, mdino, e.attr.st_nlink);
    } else {
      if (EOS_LOGS_DEBUG) {
        eos_static_debug("hlnk convert name=%s id=%#lx target local_ino=%#lx nlink0=",
                         data.name().c_str(), data.id(), local_ino, tmd->data.nlink());
      }

      e.attr.st_nlink = tmd->data.nlink();
    }
  ...
  e.attr.st_rdev = 0;
  e.attr.st_size = data.size();
  e.attr.st_blksize = 4096;
  e.attr.st_blocks = (e.attr.st_size + 511) / 512;
  e.attr.st_atime = data.atime();
  e.attr.st_mtime = data.mtime();
  e.attr.st_ctime = data.ctime();
  e.attr.ATIMESPEC.tv_sec = data.atime();
  e.attr.ATIMESPEC.tv_nsec = data.atime_ns();
  e.attr.MTIMESPEC.tv_sec = data.mtime();
  e.attr.MTIMESPEC.tv_nsec = data.mtime_ns();
  e.attr.CTIMESPEC.tv_sec = data.ctime();
  e.attr.CTIMESPEC.tv_nsec = data.ctime_ns();
  ...
  if (S_ISLNK(e.attr.st_mode)) {
    e.attr.st_size = data.target().size();
  }
...
metad::mdx::dump()
{
  ...
  snprintf(sout, sizeof(sout),
           ...
           (unsigned long) data.id(), (unsigned long)0, (unsigned int) data.mode(),
           (unsigned int) data.nlink(),
           (unsigned int) data.uid(), (unsigned int) data.gid(), (unsigned long)0,
           (unsigned long long) data.size(), (unsigned long) 4096,
           (unsigned long long) data.size() / 512,
           (unsigned long) data.atime(), (unsigned long) data.atime_ns(),
           (unsigned long) data.mtime(), (unsigned long) data.mtime_ns(),
           (unsigned long) data.ctime(), (unsigned long) data.ctime_ns());
  return sout;
}
...
metad::map_children_to_local(shared_md pmd)
{
  ...
  for (auto map = pmd->data.children().begin(); map != pmd->data.children().end(); ++map) {
    if (EOS_LOGS_DEBUG) {
	...
	if (pmd->data.children().count(nvfile)) {
	  continue;
	}
      }
    }
  ...
  pmd->data.set_nchildren(pmd->local_children().size());
  pmd->data.mutable_children()->clear();
  return ret;
}
           ...
           bool readdir)
{
  eos_static_info("ino=%#lx pino=%#lx name=%s listing=%d", ino,
                  pmd ? pmd->data.id() : 0, name, listing);
  shared_md md;
  ...
  if (!md || !md->data.id()) {
    ...
    if (pmd && (pmd->cap_count() || pmd->data.creator()) && !pmd->needs_refresh()) {
      eos_static_info("returning cap entry");
      return md;
    } else {
      eos_static_info("pmd=%#lx cap-cnt=%d", pmd ? pmd->data.id() : 0,
                      pmd ? pmd->cap_count() : 0);
      uint64_t md_pid = 0;
      mode_t md_mode = 0;
      {
        ...
        if (((!listing) || (listing && md->data.type() == md->data.MDLS)) && md->data.md_ino() &&
      ...
      if (!S_ISDIR(md_mode)) {
        ...
        if (mdmap.retrieveTS(md_pid, pmd)) {
          if (pmd && pmd->data.id() && pmd->cap_count() && !md->needs_refresh()) {
            return md;
          }
        }
      }
    ...
    if ((md->data.id() != 1) && !md->data.pid() && !md->needs_refresh()) {
    ...
    // -------------------------------------------------------------------------
  {
    ...
    if (pmd) {
       ...
       */
      pmd->Locker().Lock();
      uint64_t pmd_ino = pmd->data.md_ino();
      pmd->Locker().UnLock();
    ...
    // -------------------------------------------------------------------------
  {
    ...
    if (md->data.md_ino()) {
       ...
       */
      eos_static_info("ino=%016lx type=%d", md->data.md_ino(), md->data.type());
      rc = mdbackend->getMD(req, md->data.md_ino(), listing ? ((md->data.type() != md->data.MDLS)
                            ? 0 : md->data.clock()) : md->data.clock(),
	...
	XrdSysMutexHelper mLock(pmd->Locker());
        ...
        if (!pmd->local_children().count(encname) &&
          ...
          eos_static_info("attaching %s [%#lx] to %s [%#lx]",
                          encname.c_str(), md->data.id(),
                          pmd->data.name().c_str(), pmd->data.id());
...
metad::wait_flush(fuse_req_t req, metad::shared_md md)
{
  ...
  eos_static_info("waited for sync rc=%d bw=%#lx", md->data.err(),
                  inomap.backward(md->data.id()));

  if (!inomap.backward(md->data.id())) {
    md->Locker().Lock();
    return md->data.err();
  } else {
    md->Locker().Lock();
              ...
              bool localstore)
{
  ...
  flushentry fe(md->data.id(), authid, localstore ? mdx::LSTORE : mdx::UPDATE, req);
  fe.bind();
  mdqueue[md->data.id()]++;
  mdflushqueue.push_back(fe);
  eos_static_info("added ino=%#lx flushentry=%s queue-size=%u local-store=%d",
                  md->data.id(), flushentry::dump(fe).c_str(), mdqueue.size(), localstore);
  mdflush.Signal();
  mdflush.UnLock();
}
           ...
           std::string authid, bool localstore)
{
  ...
  if (EOS_LOGS_DEBUG)
    eos_static_debug("child=%s parent=%s inode=%016lx authid=%s localstore=%d",
                     md->data.name().c_str(),
                     pmd->data.name().c_str(), md->data.id(), authid.c_str(), localstore);
  ...
  md->Locker().UnLock();
  {
    ...
    if (!pmd->local_children().count(StringConversion::EncodeInvalidUTF8(
                                       md->data.name()))) {
      pmd->data.set_nchildren(pmd->data.nchildren() + 1);
    }

    pmd->local_children()[StringConversion::EncodeInvalidUTF8(
                            md->data.name())] = md->data.id();
    pmd->data.set_nlink(1);
    pmd->get_todelete().erase(StringConversion::EncodeInvalidUTF8(md->data.name()));
    pid = pmd->data.id();
  }
  md->Locker().Lock();
  {
    // store the local and remote parent inode
    md->data.set_pid(pmd->data.id());
    md->data.set_md_pino(pmd->data.md_ino());
    id = md->data.id();
  }
  mdflush.Lock();
  stat.inodes_backlog_store(mdqueue.size());
...
metad::add_sync(fuse_req_t req, shared_md pmd, shared_md md, std::string authid)
{
  ...
  if (EOS_LOGS_DEBUG) {
    eos_static_debug("metacache::sync ino=%016lx authid=%s op=%d", md->data.id(),
                     authid.c_str(), (int) op);
  }

  md->data.set_operation(md->data.SET);
...
//  if ((rc = mdbackend->putMD(req, reinterpret_cast<eos::fusex::md*>&(*md), authid, &(md->Locker())))) {
  if ((rc = mdbackend->putMD(req, &(md->data), authid, &(md->Locker())))) {
    eos_static_err("metad::add_sync backend::putMD failed rc=%d", rc);
    // in this case we always clean this MD record to force a refresh
    inomap.erase_bwd(md->data.id());
    md->setop_none();
    md->data.set_err(rc);

    if (md->data.id()) {
      if (mdmap.eraseTS(md->data.id())) {
        stat.inodes_dec();
        stat.inodes_ever_inc();
      }
    }
  ...
  std::string mdstream;
  std::string md_name = md->data.name();
  md->data.SerializeToString(&mdstream);
  stat.inodes_inc();
  stat.inodes_ever_inc();
  ...
  if (EOS_LOGS_DEBUG)
    eos_static_debug("child=%s parent=%s inode=%016lx authid=%s",
                     md->data.name().c_str(),
                     pmd->data.name().c_str(), md->data.id(), authid.c_str());
  ...
  flushentry fep(pmd->data.id(), authid, mdx::LSTORE, req);
  fep.bind();
  mdqueue[pmd->data.id()]++;
  mdflushqueue.push_back(fep);
  mdflush.Signal();
...
metad::begin_flush(fuse_req_t req, shared_md emd, std::string authid)
{
  ...
  md->data.set_md_ino(emd->data.md_ino());

//    if ((rc = mdbackend->putMD(req, &(*md), authid, 0))) {
//    if ((rc = mdbackend->putMD(req, reinterpret_cast<eos::fusex::md*>(&(*md)), authid, 0))) {
    if ((rc = mdbackend->putMD(req, &(md->data), authid, 0))) {
              ...
              bool upstream)
{
  ...
  if (EOS_LOGS_DEBUG)
    eos_static_debug("child=%s parent=%s inode=%#lx upstreaqm=%d",
                     md->data.name().c_str(),
                     pmd->data.name().c_str(), md->data.id(), upstream);
  ...
  md->data.set_mtime(ts.tv_sec);
  md->data.set_mtime_ns(ts.tv_nsec);
  md->setop_delete();

  if ( EosFuse::Instance().Config().options.hide_versions && EosFuse::Instance().mds.supports_hideversion() ) {
    // indicate the MGM to remove also all versions
    md->data.set_opflags(eos::fusex::md::DELETEVERSIONS);
  }
  ...
  md->Locker().UnLock();
  {
    ...
    pmd->get_todelete()[eos::common::StringConversion::EncodeInvalidUTF8(
                          name)] = md->data.id();
    pmd->data.set_mtime(ts.tv_sec);
    pmd->data.set_mtime_ns(ts.tv_nsec);
  }
  md->Locker().Lock();
  ...
  flushentry fe(md->data.id(), authid, mdx::RM, req);
  fe.bind();
  flushentry fep(pmd->data.id(), authid, mdx::LSTORE, req);
  fep.bind();
  mdflush.Lock();
          ...
          std::string newname, std::string authid1, std::string authid2)
{
  if (EOS_LOGS_DEBUG)
    eos_static_debug("child=%s new-name=%s parent=%s newparent=%s inode=%016lx",
                     md->data.name().c_str(),
                     newname.c_str(),
                     p1md->data.name().c_str(), p2md->data.name().c_str(), md->data.id());

  XrdSysMutexHelper mLock(md->Locker());
  struct timespec ts;
  eos::common::Timing::GetTimeSpec(ts);

  if (p1md->data.id() != p2md->data.id()) {
    ...
    if (!p2md->local_children().count(
      ...
      p2md->data.set_nchildren(p2md->data.nchildren() + 1);
    }
    ...
    p1md->local_children().erase(eos::common::StringConversion::EncodeInvalidUTF8(
                                   md->data.name()));
    p1md->data.set_nchildren(p1md->data.nchildren() - 1);
    p1md->data.set_mtime(ts.tv_sec);
    p1md->data.set_mtime_ns(ts.tv_nsec);
    p1md->data.clear_pmtime();
    p1md->data.clear_pmtime_ns();
    p1md->data.set_ctime(ts.tv_sec);
    p1md->data.set_ctime_ns(ts.tv_nsec);
    p2md->data.set_mtime(ts.tv_sec);
    p2md->data.set_mtime_ns(ts.tv_nsec);
    p2md->data.clear_pmtime();
    p2md->data.clear_pmtime_ns();
    p2md->data.set_ctime(ts.tv_sec);
    p2md->data.set_ctime_ns(ts.tv_nsec);
    md->data.set_name(newname);
    md->data.set_pid(p2md->data.id());
    md->data.set_md_pino(p2md->data.md_ino());
    p1md->get_todelete()[eos::common::StringConversion::EncodeInvalidUTF8(
                           oldname)] = 0; //md->data.id(); // make it known as deleted
    ...
    p1md->local_children().erase(eos::common::StringConversion::EncodeInvalidUTF8(
                                   md->data.name()));
    p1md->get_todelete()[eos::common::StringConversion::EncodeInvalidUTF8(
                           md->data.name())] = md->data.id(); // make it known as deleted
    p2md->get_todelete().erase(eos::common::StringConversion::EncodeInvalidUTF8(
                                 newname)); // the new target is not deleted anymore
    p2md->local_enoent().erase(newname); // remove a possible enoent entry
    md->data.set_name(newname);
  ...
  md->data.clear_pmtime();
  md->data.clear_pmtime_ns();
  md->data.set_ctime(ts.tv_sec);
  md->data.set_ctime_ns(ts.tv_nsec);
  md->data.set_mv_authid(authid1); // store also the source authid
  mdflush.Lock();
  ...
  flushentry fe1(p1md->data.id(), authid1, mdx::UPDATE, req);
  fe1.bind();
  mdqueue[p1md->data.id()]++;
  mdflushqueue.push_back(fe1);

  if (p1md->data.id() != p2md->data.id()) {
    flushentry fe2(p2md->data.id(), authid2, mdx::UPDATE, req);
    fe2.bind();
    mdqueue[p2md->data.id()]++;
    mdflushqueue.push_back(fe2);
  }

  flushentry fe(md->data.id(), authid2, mdx::UPDATE, req);
  fe.bind();
  mdqueue[md->data.id()]++;
  mdflushqueue.push_back(fe);
  stat.inodes_backlog_store(mdqueue.size());
...
//  int rc = mdbackend->rmRf(req, reinterpret_cast<eos::fusex::md*>(&(*md)));
  int rc = mdbackend->rmRf(req, &(md->data));
  return rc;
}
...
metad::getlk(fuse_req_t req, shared_md md, struct flock* lock)
{
  ...
  md->data.mutable_flock()->set_pid(fuse_req_ctx(req)->pid);
  md->data.mutable_flock()->set_len(lock->l_len);
  md->data.mutable_flock()->set_start(lock->l_start);
  md->data.set_operation(md->data.GETLK);

  switch (lock->l_type) {
  case F_RDLCK:
    md->data.mutable_flock()->set_type(eos::fusex::lock::RDLCK);
    break;

  case F_WRLCK:
    md->data.mutable_flock()->set_type(eos::fusex::lock::WRLCK);
    break;

  case F_UNLCK:
    md->data.mutable_flock()->set_type(eos::fusex::lock::UNLCK);
    break;
...
//  int rc = mdbackend->doLock(req, reinterpret_cast<eos::fusex::md&>(*md), &(md->Locker()));
  int rc = mdbackend->doLock(req, md->data, &(md->Locker()));

  // digest the response
  if (!rc) {
    // store the md->flock response into the flock structure
    lock->l_pid = md->data.flock().pid();
    lock->l_len = md->data.flock().len();
    lock->l_start = md->data.flock().start();
    lock->l_whence = SEEK_SET;

    switch (md->data.flock().type()) {
...
metad::setlk(fuse_req_t req, shared_md md, struct flock* lock, int sleep)
{
  ...
  if (sleep) {
    md->data.set_operation(md->data.SETLKW);
  } else {
    md->data.set_operation(md->data.SETLK);
  }
  ...
  if (lock->l_type == F_UNLCK) {
    ...
    for (auto it = md->LockTable().begin(); it != md->LockTable().end(); ++it) {
      if (it->l_pid == (pid_t) md->data.flock().pid()) {
        backend_call = true;
      }
    }
  }
...
//    rc = mdbackend->doLock(req, reinterpret_cast<eos::fusex::md&>(*md), &(md->Locker()));
    rc = mdbackend->doLock(req, md->data, &(md->Locker()));
...
/* -------------------------------------------------------------------------- */
{
  eos_static_debug("id=%16x", md->data.id());
  ...
  for (auto it = md->local_children().begin();
    ...
    if (mdmap.retrieveTS(it->second, cmd)) {
      ...
      if (!S_ISDIR(cmd->data.mode())) {
        if (!in_flush && !EosFuse::Instance().datas.has(cmd->data.id())) {
  ...
  if (EosFuse::Instance().Config().options.md_kernelcache) {
    for (auto it = inval_entry_name.begin(); it != inval_entry_name.end(); ++it) {
      kernelcache::inval_entry(md->data.id(), *it);
    }
  }

  md->Locker().Lock();
  md->data.set_type(md->data.MD);
  md->data.set_creator(false);
  md->cap_count_reset();
  md->data.set_nchildren(md->local_children().size());
...
//        md->data.CopyFrom(cont.md_());
	...
	if (d_md) {
          ...
          md->data.set_size(d_md->data.size());
          md->data.set_mtime(d_md->data.mtime());
          md->data.set_mtime_ns(d_md->data.mtime_ns());
        }
      } else {
    ...
    md->data.set_pid(p_ino);
    md->data.set_id(ino);
    md->clear_refresh();
    eos_static_info("store local pino=%016lx for %016lx", md->data.pid(), md->data.id());
    ...
    for (auto map = cont.md_map_().md_map_().begin();
      ...
      if (mdmap.retrieveTS(ino, md)) {
        ...
        eos_static_debug("lock mdmap");
        {
          ...
          if (child) {
            eos_static_debug("case 1 %s", md->data.name().c_str());
            eos::fusex::md::TYPE mdtype = md->data.type();
            size_t local_size = md->data.size();
            uint64_t local_mtime = md->data.mtime();
            uint64_t local_mtime_ns = md->data.mtime_ns();

	    md->data.CopyFrom(map->second);
	    md->clear_refresh();
	    ...
	    if (d_md) {
	      ...
	      if (has_flush(ino)) {
		md->data.set_size(local_size);
		md->data.set_mtime(local_mtime);
		md->data.set_mtime_ns(local_mtime_ns);
	      }
	    }
	    md->data.set_nchildren(md->local_children().size());
	    // if this object was a listing type, keep that
	    md->data.set_type(mdtype);
          } else {
            ...
            if (!mdqueue.count(md->data.id())) {
              eos_static_debug("case 2 %s id %#lx", md->data.name().c_str(), md->data.id());
              mdflush.UnLock();
              ...
              eos_static_debug("case 3 %s children=%d listing=%d", md->data.name().c_str(),
                               map->second.children().size(), listing);
              mdflush.UnLock();
              todelete = md->get_todelete();
              // copy only the listing
              md->data.mutable_children()->clear();

              for (auto it = map->second.children().begin();
                   it != map->second.children().end(); ++it) {
                (*md->data.mutable_children())[eos::common::StringConversion::EncodeInvalidUTF8(
                                            it->first)] = it->second;
              }
          ...
          if (!child) {
            ...
            if (!pmd->cap_count()) {
              if (EOS_LOGS_DEBUG) {
                eos_static_debug("clearing out %0016lx", pmd->data.id());
              }
        ...
        *md = map->second;
        ...
        if (EOS_LOGS_DEBUG) {
          eos_static_debug("store md for local-ino=%016lx remote-ino=%016lx type=%d -",
                           (long) new_ino, (long) map->first, md->data.type());
        }
    ...
    if (pmd && listing) {
      ...
      pmd->data.set_type(pmd->data.MDLS);
    }
...

      {
        ...
        if (md->data.id()) {
          uint64_t removeentry = 0;
          {
            ...
            if (op == metad::mdx::RM) {
              md->data.set_operation(md->data.DELETE);
            } else {
              md->data.set_operation(md->data.SET);
            }
...
//              if ((rc = mdbackend->putMD(f_id, reinterpret_cast<eos::fusex::md*>(&(*md)), authid, &(md->Locker())))) {
              if ((rc = mdbackend->putMD(f_id, &(md->data), authid, &(md->Locker())))) {
                eos_static_err("metacache::flush backend::putMD failed rc=%d", rc);
                // we just set an error code
                //! inomap.erase_bwd(md->data.id());
                //! removeentry=md->data.id();
                md->data.set_err(rc);
              } else {
                inomap.insert(md->data.md_ino(), md->data.id());
              }
  ...
  while (!assistant.terminationRequested()) {
    ...
    if (!(cnt % 256)) {
      ...
      for (auto it = mdmap.begin(); it != mdmap.end();) {
        ...
        if ((!mdmap.count(it->second->data.pid())) &&
            (!S_ISDIR(it->second->data.mode()) || it->second->deleted())) {
          eos_static_debug("removing orphaned inode from mdmap ino=%#lx path=%s",
                           it->first, it->second->data.fullpath().c_str());
...
metad::isChild(shared_md potentialChild, fuse_ino_t parentId)
{
  ...
  if (potentialChild->data.id() == 1 || potentialChild->data.id() == 0) {
    return false;
  }

  if (potentialChild->data.id() == parentId) {
    return true;
  }

  shared_md pmd;

  if (!mdmap.retrieveTS(potentialChild->data.pid(), pmd)) {
    eos_static_warning("could not lookup parent ino=%d of %d when determining lock order..",
                       potentialChild->data.pid(), potentialChild->data.id());
    return false;
  }
...
{
  if (md->data.id() == 1 || md->data.id() == 0) {
    return 1;
  }

  fuse_ino_t pino = md->data.pid();
		...
		EosFuse::Instance().mds.hideversion = rsp.config_().hideversion();
            ...
            if (rsp.type() == rsp.MD) {
              ...
              if (mdmap.retrieveTS(ino, md)) {
                ...
                md->Locker().Lock();
                bookingsize = rsp.md_().size() - md->data.size();
                md_clientid = rsp.md_().clientid();
                eos_static_info("md-update: %s %s", md->data.name().c_str(),
                                rsp.md_().name().c_str());

                // check if this implies a rename
                if (md->data.name() != rsp.md_().name()) {
                  old_name = rsp.md_().name();
                }

                // verify that this record is newer than
                if (rsp.md_().clock() >= md->data.clock()) {
                  eos_static_info("overwriting clock MD %#lx => %#lx", md->data.clock(),
                                  rsp.md_().clock());
                  *md = rsp.md_();
                  md->data.set_creator(false);
                  md->data.set_bc_time(time(NULL));
                } else {
                  eos_static_warning("keeping clock MD %#lx => %#lx", md->data.clock(),
                                     rsp.md_().clock());
                }

                md->data.clear_clientid();
                pino = inomap.forward(md->data.md_pino());
                md->data.set_id(ino);
                md->data.set_pid(pino);
                mode = md->data.mode();
                ...
                *md = rsp.md_();
                ...
                if (pino && mdmap.retrieveTS(pino, pmd)) {
                  if (md->data.pt_mtime()) {
                    pmd->data.set_mtime(md->data.pt_mtime());
                    pmd->data.set_mtime_ns(md->data.pt_mtime_ns());
                  }
                  ...
                  if (EosFuse::Instance().Config().options.md_kernelcache) {
                    ...
                    kernelcache::inval_entry(pino, md->data.name());
                    kernelcache::inval_inode(pino, false);
		    XrdSysMutexHelper mLock(pmd->Locker());
                    pmd->local_enoent().erase(md->data.name());
                  }
                } else {
...
metad::pmap::swap_out(shared_md md)
{
  ...
  if (!md->data.SerializeToString(&mdstream)) {
    return EFAULT;
  }
  ...
  if (store) {
    ...
    md_key += std::to_string(md->data.id());
    ...
    md_state_key += std::to_string(md->data.id());
...
metad::pmap::swap_in(fuse_ino_t ino, shared_md md)
{
  ...
  if (store) {
    ...
    if (!md->data.ParseFromString(mdstream)) {
      return EFAULT;
    }
