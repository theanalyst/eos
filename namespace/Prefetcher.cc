...
void Prefetcher::stageFileMDWithParents(IFileMD::id_t id)
{
  ...
  mUris.emplace_back(std::move(fut).thenValue(std::bind(&Prefetcher::prefetchFileUri, this, _1)));
}
...
void Prefetcher::stageContainerMDWithParents(IContainerMD::id_t id)
{
  ...
  mUris.emplace_back(std::move(fut).thenValue(std::bind(&Prefetcher::prefetchContUri, this, _1)));
}
