...
com_space(char* arg1)
{
  ...
  if (subcommand == "tracker") {
    in = "mgm.cmd=space&mgm.subcmd=tracker&mgm.space=default";
    ok = true;
  }
...
com_space_usage:
  ...
  fprintf(stdout,
          "       space config <space-name> space.tgc.freespaceqryperiod=<#>    : delay in seconds between free space queries for the tape-aware GC [ default=%lu ]\n", eos::mgm::tgc::TGC_DEFAULT_FREE_BYTES_QRY_PERIOD_SECS);
  fprintf(stdout,
