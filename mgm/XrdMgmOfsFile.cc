...
/*----------------------------------------------------------------------------*/
{
  ...
  std::string creation_path = path;
  {
    ...
    if (!AccessChecker::checkContainer(dmd.get(), aclC, X_OK, vid)) {
    ...
    // fusex creates files in an inconvenient place, messing up
    // the initial writes if the owner is not granted write access,
    // seen with .git mode 0444 pack files
